...
app.include_router(v1.endpoints, prefix=f"{settings.BASE_PATH}/v1")
app.include_router(health_check.router, prefix=f"{settings.BASE_PATH}/health")
app.include_router(auth.router, prefix=f"{settings.BASE_PATH}/auth")


@app.exception_handler(404)
async def not_found(request: Request, exc: HTTPException):
    return UJSONResponse({"message": f"Coul'd not found {request.url}"})
