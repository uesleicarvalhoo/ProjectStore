...
from src.core.models.balance import QueryBalance
...
async def index(
    ...
    end_date = now_datetime().today()
    start_date = date(day=1, month=end_date.month, year=end_date.year)

    clients = controller.client.get_all(session, QueryClient(), context=context)
    orders = controller.order.get_all(session, QueryOrder(start_date=start_date, end_date=end_date), context=context)
    balances = controller.balance.get_all(
        session, QueryBalance(start_date=start_date, end_date=end_date), context=context
    )
