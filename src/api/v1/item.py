...
async def get(item_id: UUID, session: Session = Depends(make_session)):
...
async def create(item: CreateItem, fiscal_note_id: UUID, session: Session = Depends(make_session)):
    return item.create(session, item, fiscal_note_id=fiscal_note_id, context=Context.API)


@router.delete("/{item_id}", response_model=Item)
async def delete(item_id: UUID, session: Session = Depends(make_session)):
