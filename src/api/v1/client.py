...
from src.core.models import Client, Context, CreateClient, QueryClient
...
async def get_by_id(
    client_id: UUID, session: Session = Depends(make_session), context: Context = Depends(api_context_manager)
):
...
async def create(
    schema: CreateClient, session: Session = Depends(make_session), context: Context = Depends(api_context_manager)
):
